...
-- ( >шутка< Создание резервной копии данных из users)
CREATE TABLE users_backup AS SELECT * FROM users;

-- Удаление таблицы users
DROP TABLE IF EXISTS users;

-- Создание таблицы users заново с первоначальной структурой
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    firstname VARCHAR(50),
    lastname VARCHAR(50) COMMENT 'Фамилия',
...
-- Восстановление данных из резервной копии в таблицу users
...
DROP PROCEDURE IF EXISTS MoveUserToOld;
DELIMITER //
CREATE PROCEDURE MoveUserToOld(IN user_id_to_move BIGINT)
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'An error occurred during the user move.';
    END;

    START TRANSACTION;
    
    SELECT firstname, lastname, email
    INTO user_firstname, user_lastname, user_email
    FROM users
    WHERE id = user_id_to_move;
    
    INSERT INTO users_old (id, firstname, lastname, email)
    VALUES (user_id_to_move, user_firstname, user_lastname, user_email);
...
DELIMITER //
CREATE TRIGGER after_insert_communities
...
(1, 2, 'Hello Frederick!'),
(2, 1, 'Hi Frederik! Happy to hear from you! Have you heard about Austyn?'),
(1, 2, 'Yeah, he moved!'),
(3, 1, 'Yeah, I have moved');
